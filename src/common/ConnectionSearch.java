...
public class ConnectionSearch implements Search{
    ...
    private ConnectionSearch(LineFactory lineFactory, StopFactory stopFactory){
        ...
        lineFactory.setStops(stops);
        lines = new Lines(lineFactory);
    }
    ...
    @Override
    public ConnectionData searchAndReserve(StopName from, StopName to, Time when) {
        return search(from, to, when);
    }
}
